...
export class DeleteFornecedorService {
    ...
    async DeleteFornecedor(id_fornecedor: String){

        this.logger.log(`Tentativa de exclusão do fornecedor ${id_fornecedor}`)

            const fornecedorExiste = await Fornecedor.findOne({where: {id_fornecedor}});
            ...
            await Fornecedor.destroy({where: {id_fornecedor}})
        }
}